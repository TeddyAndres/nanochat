...
# Union IO remapping for grad-accum persistent slots is performed with plain
# CPU-side advanced indexing below. A previous attempt to @torch.compile it
# (fullgraph=True) caused repeated recompiles (and FailOnRecompileLimitHit)
# because both the indices length and (in some versions) the slot_map prefix
# length legitimately vary across grad-accum windows and microsteps.
# There is no meaningful perf win from compiling this tiny CPU op, so we
# keep it eager. Real Phase 1 compile-boundary work targets the active_vocab
# dict handoff and other GPU-side prep logic.


@dataclass
class PersistentSlotPrefetchState:
    """Legacy / future carrier for precomputed persistent slot state.

    Phase 2+ primarily uses PersistentSlotTransition (below) as the rich
    result object exchanged between the (background) compute and the main-thread
    apply. This dataclass is retained for the prefetch future slot and can be
    unified later if desired.
    """
...
@dataclass
class PersistentSlotTransition:
    """Pure result of a persistent slot window transition computation.

    Produced by _compute_persistent_slot_state_cpu (or its for_meta wrapper)
    which is now side-effect free. Applied atomically on the main thread by
    _apply_persistent_slot_transition.

    This eliminates the race that previously forced the heavy logic onto the
    foreground path when grad_accum_steps > 1 + fixed overlap reuse.
    """
    ...
    # Pre-computed values that should be copied into
    # _grad_accum_wte_local_to_slot_cpu[:n] for the window.
    grad_accum_local_to_slot_cpu: torch.Tensor
...
class DynamicVocabStep:
    ...
    prep_active_vocab_build_ms: float = 0.0  # cost of building dict passed to compiled model
    prep_persistent_slot_ms: float = 0.0     # time in the new-window persistent slot remapping logic
...
class DynamicVocabRuntime:
    ...
    def __init__(
        ...
        if self.fixed_u_mode:
            ...
            # Full-size buffer for the local-to-slot map used in union remapping.
            # Allocated to max capacity so the compiled helper always sees a consistent shape.
            self._grad_accum_wte_local_to_slot_cpu = torch.full(
                (self.grad_accum_u_max,), -1, dtype=torch.long
            )
            self._grad_accum_union_slot_remap_active = False
            self._grad_accum_slot_positions_cpu = torch.arange(self.grad_accum_u_max, dtype=torch.long)
            self._prev_lm_head_global_to_row_cpu = torch.full(
                (self.model.config.vocab_size,), -1, dtype=torch.long
            )
            self._prev_lm_head_global_to_slot_cpu = torch.full(
                (self.model.config.vocab_size,), -1, dtype=torch.long
            )
            self._grad_accum_global_to_local_work_cpu = torch.full(
                (self.model.config.vocab_size,), -1, dtype=torch.long
            )
            ...
            self._fixed_active_vocab_with_logit_mask = {
                **self.fixed_active_vocab,
    ...
    def _as_cpu_long(self, tensor: torch.Tensor) -> torch.Tensor:
        if tensor.device.type == "cpu" and tensor.dtype == torch.long:
            return tensor
        return tensor.detach().to(device="cpu", dtype=torch.long)

    def _as_cpu_bool(self, tensor: torch.Tensor) -> torch.Tensor:
        if tensor.device.type == "cpu" and tensor.dtype == torch.bool:
            return tensor
        return tensor.detach().to(device="cpu", dtype=torch.bool)

    def _grad_accum_slot_positions_view(self, count: int) -> torch.Tensor:
        if count <= 0:
            return self._empty_long_cpu()
        return self._grad_accum_slot_positions_cpu[:count]

    def _grad_accum_slot_positions_unique(self, count: int) -> torch.Tensor:
        if count <= 0:
            return self._empty_long_cpu()
        return self._grad_accum_slot_positions_cpu[:count]

    def _assert_main_thread_for_slot_mutation(self, context: str = "") -> None:
        """Debug guard (Phase 2+): cheap no-op unless NANOCHAT_SPARSE_DEBUG=1.

        Asserts that we are not mutating the live fixed-slot maps from a
        background thread (the original source of the "missing live fixed-U rows"
        race when persistent slot prefetch was previously enabled).
        """
        if os.environ.get("NANOCHAT_SPARSE_DEBUG", "0") not in ("1", "true", "True"):
            return
        # Best-effort: if we are inside a ThreadPoolExecutor worker, the current
        # thread name usually contains "ThreadPoolExecutor".
        import threading
        name = threading.current_thread().name
        if "ThreadPoolExecutor" in name or "Thread-" in name:
            raise RuntimeError(
                f"Persistent slot map mutation attempted from background thread "
                f"({name}). Context: {context}. This would have caused the race. "
                "All mutations must go through the main-thread apply path."
            )
        # No action needed on main thread — guard passes silently when debug is off or we're on main.

    def _same_grad_accum_window_ids(self, grad_accum_ids_cpu: torch.Tensor) -> bool:
        if not self._grad_accum_live or self._grad_accum_ids_cpu is None:
            return False
        if grad_accum_ids_cpu is self._grad_accum_ids_cpu:
            return True
        return bool(self._grad_accum_ids_cpu.equal(grad_accum_ids_cpu))

    def _align_slot_to_global_cpu(self, slot_to_global_cpu: torch.Tensor) -> torch.Tensor:
        manifest_u = int(slot_to_global_cpu.numel())
        if manifest_u > self.fixed_u_max:
            raise ValueError(
                f"Manifest fixed-U metadata exceeds runtime capacity: manifest_u_max={manifest_u}, runtime_fixed_u_max={self.fixed_u_max}"
            )
        slot_to_global_cpu = self._as_cpu_long(slot_to_global_cpu)
        assert self.fixed_slot_to_global_cpu is not None
        if manifest_u == self.fixed_u_max:
            return slot_to_global_cpu
        self.fixed_slot_to_global_cpu.fill_(-1)
        if manifest_u > 0:
            self.fixed_slot_to_global_cpu[:manifest_u].copy_(slot_to_global_cpu)
        return self.fixed_slot_to_global_cpu

    def _align_active_mask_cpu(self, active_mask_cpu: torch.Tensor) -> torch.Tensor:
        manifest_u = int(active_mask_cpu.numel())
        active_mask_cpu = self._as_cpu_bool(active_mask_cpu)
        assert self.fixed_active_mask_cpu is not None
        if manifest_u == self.fixed_u_max:
            return active_mask_cpu
        self.fixed_active_mask_cpu.zero_()
        if manifest_u > 0:
            self.fixed_active_mask_cpu[:manifest_u].copy_(active_mask_cpu)
        return self.fixed_active_mask_cpu

    def _fill_prev_lm_head_global_to_row_cpu(
        self,
        prev_lm_head_ids_cpu: torch.Tensor,
        prev_lm_head_slot_ids_cpu: torch.Tensor,
    ) -> torch.Tensor:
        buf = self._prev_lm_head_global_to_row_cpu
        buf.fill_(-1)
        if prev_lm_head_ids_cpu.numel() > 0:
            buf[prev_lm_head_ids_cpu] = self._grad_accum_slot_positions_view(prev_lm_head_ids_cpu.numel())
        return buf

    def _fill_prev_lm_head_global_to_slot_cpu(
        self,
        prev_lm_head_ids_cpu: torch.Tensor,
        prev_lm_head_slot_ids_cpu: torch.Tensor,
    ) -> torch.Tensor:
        buf = self._prev_lm_head_global_to_slot_cpu
        buf.fill_(-1)
        if prev_lm_head_ids_cpu.numel() > 0:
            buf[prev_lm_head_ids_cpu] = prev_lm_head_slot_ids_cpu
        return buf
    ...
    def _invalidate_fixed_live_state(self):
        ...
        if self._grad_accum_wte_local_to_slot_cpu is not None:
            self._grad_accum_wte_local_to_slot_cpu.fill_(-1)
        self._grad_accum_union_slot_remap_active = False
        if self.fixed_active_mask_cpu is not None:
            self.fixed_active_mask_cpu.zero_()
        if self.fixed_logit_mask is not None:
            self.fixed_logit_mask.zero_()
        # Clear any pending persistent slot prefetch (safe to drop on live state invalidation)
        self._pending_persistent_slot_future = None
        self._pending_persistent_slot_key = None
    ...
    def _start_grad_accum_window(self, grad_accum_ids_cpu: torch.Tensor):
        ...
        if grad_accum_count > 0:
            global_to_local[grad_accum_ids_cpu] = self._grad_accum_slot_positions_view(grad_accum_count)
    ...
    def _build_fixed_prefetch_requests(self, step_meta: dict) -> dict[str, torch.Tensor]:
        active_ids_cpu = self._as_cpu_long(step_meta["active_ids_cpu"])
        grad_accum_ids_cpu = self._as_cpu_long(step_meta.get("grad_accum_ids_cpu", active_ids_cpu))
        grad_accum_steps = int(step_meta.get("grad_accum_steps", 1))
        stage_ids_cpu = self._as_cpu_long(step_meta["stage_ids_cpu"])
        ...
        if grad_accum_steps > 1:
            ...
            if (not self.disable_fixed_overlap_reuse) and self._fixed_live_state and grad_accum_ids_cpu.numel() > 0:
                ...
                if prev_lm_head_slot_ids_cpu.numel() > 0:
                    ...
                    prev_lm_head_global_to_slot_cpu = self._fill_prev_lm_head_global_to_slot_cpu(
    ...
    def _compute_persistent_slot_state_cpu(
        ...
        preserve_resident_grads: bool,
        union_input_tables: bool,
        stage_ids_cpu: torch.Tensor,
        stage_slot_ids_cpu: torch.Tensor,
    ) -> PersistentSlotTransition:
        """Pure (side-effect-free) version.

        Computes arriving/leaving decisions, the required local_to_slot slice,
        and lm_head deferred writeback list for a grad-accum window transition.
        All heavy index work can now safely run in the prefetch executor.

        The caller (on the main thread) must subsequently call
        _apply_persistent_slot_transition(result) to commit the map changes.
        """
        _t0 = time.perf_counter()

        # Defaults (empty tensors)
        empty = self._empty_long_cpu()
        transition = PersistentSlotTransition(
            ...
            grad_accum_local_to_slot_cpu=empty,
            used_persistent_input_slots=False,
            prep_persistent_slot_ms=0.0,
        )

        if not (self._fixed_live_state and not self.disable_fixed_overlap_reuse):
            transition.prep_persistent_slot_ms = (time.perf_counter() - _t0) * 1000.0
            return transition

        assert self.fixed_input_slot_to_global_cpu is not None

        # --- Pure decision logic (no mutations) ---
        ...
        # Build the *would-be* final global_to_slot values for the new window ids
        # (this is what used to be read after the mutations)
        n = grad_accum_ids_cpu.numel()
        local_to_slot = torch.empty(n, dtype=torch.long)

        # Survivors keep their previous slots
        if _surviving_input_slots.numel() > 0:
            survivor_mask = ~_arriving_input_mask
            local_to_slot[survivor_mask] = _surviving_input_slots

        # Arrivers get the slots we decided for them
        if _n_arriving > 0:
            arriver_mask = _arriving_input_mask
            local_to_slot[arriver_mask] = _arriving_input_slot_ids

        # The compute is now pure — no map mutations here.
        # The caller applies via _apply_persistent_slot_transition.

        deferred_writeback_ids_cpu = _leaving_input_ids
        deferred_writeback_slot_ids_cpu = _leaving_input_slot_ids[:_leaving_input_ids.numel()] if _leaving_input_ids.numel() > 0 else empty
        input_stage_ids_cpu = _arriving_input_ids
        input_stage_slot_ids_cpu = _arriving_input_slot_ids

        # lm_head deferred writeback detection (pure)
        ...
        transition = PersistentSlotTransition(
            ...
            grad_accum_local_to_slot_cpu=local_to_slot,
            used_persistent_input_slots=True,
            prep_persistent_slot_ms=(time.perf_counter() - _t0) * 1000.0,
        )
        return transition

    def _compute_persistent_slot_state_for_meta(self, step_meta: dict) -> PersistentSlotTransition:
        ...
        """
        grad_accum_ids_cpu = self._as_cpu_long(step_meta.get("grad_accum_ids_cpu", step_meta.get("active_ids_cpu")))
        ...
        stage_slot_ids_cpu = self._as_cpu_long(
            step_meta.get("stage_slot_ids_cpu", self._grad_accum_slot_positions_unique(grad_accum_ids_cpu.numel() if grad_accum_ids_cpu is not None else 0))
        )
    ...
    def _apply_persistent_slot_transition(self, transition: "PersistentSlotTransition") -> None:
        """Atomic main-thread apply of a PersistentSlotTransition.

        Performs exactly the mutations that used to live inside the compute
        function. Must only ever be called from the main thread (the debug
        guard will catch violations when NANOCHAT_SPARSE_DEBUG=1).
        """
        if not transition.used_persistent_input_slots:
            return

        self._assert_main_thread_for_slot_mutation("_apply_persistent_slot_transition")

        # --- Apply input table slot assignments (arriving / leaving) ---
        if transition.deferred_writeback_ids_cpu.numel() > 0:
            self._fixed_input_global_to_slot_cpu[transition.deferred_writeback_ids_cpu] = -1
        if transition.deferred_writeback_slot_ids_cpu.numel() > 0:
            self.fixed_input_slot_to_global_cpu[transition.deferred_writeback_slot_ids_cpu] = -1
        if transition.input_stage_ids_cpu.numel() > 0:
            self._fixed_input_global_to_slot_cpu[transition.input_stage_ids_cpu] = transition.input_stage_slot_ids_cpu
            self.fixed_input_slot_to_global_cpu[transition.input_stage_slot_ids_cpu] = transition.input_stage_ids_cpu

        # local_to_slot for the current window's union remap
        n = transition.grad_accum_local_to_slot_cpu.numel()
        assert self._grad_accum_wte_local_to_slot_cpu is not None
        if n > 0:
            self._grad_accum_wte_local_to_slot_cpu[:n] = transition.grad_accum_local_to_slot_cpu
        if n < self.grad_accum_u_max:
            self._grad_accum_wte_local_to_slot_cpu[n:].fill_(-1)
        self._grad_accum_union_slot_remap_active = True

        # Note: lm_head map mutations for the new window (and the GPU reuse copies)
        # are handled in the caller (_prepare_fixed_step) after this apply, exactly
        # as they were before. The compute only ever computed the *deferred writeback*
        # list for lm_head.
    ...
    def _prepare_fixed_step(
        ...
        prefetched_persistent_slot_state: Optional[object] = None,  # PersistentSlotTransition (preferred) or legacy dict
        prefetched_persistent_slot_wait_ms: float = 0.0,
        prefetched_persistent_slot_hit: int = 0,
    ) -> DynamicVocabStep:
        ...
        active_slot_ids_cpu = self._as_cpu_long(step_meta["active_slot_ids_cpu"])
        ...
        preserve_resident_grads = grad_accum_steps > 1 and self._same_grad_accum_window_ids(grad_accum_ids_cpu)
        ...
        # Cloud/warm/cold grad accum caches removed (dead experimental code).
        # We only keep hot activation counts for the still-supported first-hot unembedding LR feature.
        active_union_row_ids_cpu = self._empty_long_cpu()
        union_cold_steps_cpu = self._empty_long_cpu()
        cold_steps_cpu = self._empty_long_cpu()
        hot_activation_counts_cpu = self._empty_long_cpu()
        if grad_accum_steps > 1:
            assert self._grad_accum_global_to_local_cpu is not None
            if active_ids_cpu.numel() > 0:
            ...
            cold_steps_cpu, hot_activation_counts_cpu = self._capture_cold_steps_cpu(active_ids_cpu)
        active_mask_cpu = self._align_active_mask_cpu(step_meta["active_mask_cpu"])
        slot_to_global_cpu = self._align_slot_to_global_cpu(step_meta["slot_to_global_cpu"])
        stage_ids_cpu = self._as_cpu_long(step_meta["stage_ids_cpu"])
        stage_slot_ids_cpu = self._as_cpu_long(step_meta["stage_slot_ids_cpu"])
        writeback_ids_cpu = self._as_cpu_long(step_meta["writeback_ids_cpu"])
        writeback_slot_ids_cpu = self._as_cpu_long(step_meta["writeback_slot_ids_cpu"])
        is_last_step = bool(step_meta["is_last_step"])
        cloud_slot_ids_cpu = self._empty_long_cpu()
        lm_head_active_ids_cpu = grad_accum_ids_cpu if grad_accum_steps > 1 else active_ids_cpu
        grad_accum_window_slot_positions_cpu = (
            self._grad_accum_slot_positions_unique(grad_accum_ids_cpu.numel()) if grad_accum_steps > 1 else self._empty_long_cpu()
        )
        lm_head_active_slot_ids_cpu = grad_accum_window_slot_positions_cpu if grad_accum_steps > 1 else active_slot_ids_cpu
        ...
        grad_accum_stage_slot_ids_cpu = grad_accum_window_slot_positions_cpu
        ...
        prep_persistent_slot_ms = 0.0  # NEW: time in the persistent slot remapping logic (hot on window boundaries)
        ...
        if union_input_tables:
            ...
            input_stage_slot_ids_cpu = (
                self._empty_long_cpu() if preserve_resident_grads else grad_accum_window_slot_positions_cpu
            )
        _t0_cpu_map = time.perf_counter()

        # Prefer precomputed persistent slot state from the prefetch executor
        ...
        if grad_accum_steps > 1 and not preserve_resident_grads and grad_accum_ids_cpu.numel() > 0 and self._fixed_live_state and not self.disable_fixed_overlap_reuse:
            if prefetched_persistent_slot_hit and prefetched_persistent_slot_state is not None:
                state = prefetched_persistent_slot_state
                ...
                prep_persistent_slot_ms = getattr(state, "prep_persistent_slot_ms", 0.0)

            # Normalize to dataclass (supports legacy dicts during transition)
            if isinstance(state, dict):
                state = PersistentSlotTransition(
                    ...
                    grad_accum_local_to_slot_cpu=state.get("_grad_accum_local_to_slot_cpu", self._empty_long_cpu()),
                    used_persistent_input_slots=state.get("_used_persistent_input_slots", False),
                    prep_persistent_slot_ms=prep_persistent_slot_ms,
                )

            # Commit the (previously computed) map changes on the main thread only.
            self._apply_persistent_slot_transition(state)

            deferred_writeback_ids_cpu = state.deferred_writeback_ids_cpu
            deferred_writeback_slot_ids_cpu = state.deferred_writeback_slot_ids_cpu
            deferred_lm_head_writeback_ids_cpu = state.deferred_lm_head_writeback_ids_cpu
            deferred_lm_head_writeback_slot_ids_cpu = state.deferred_lm_head_writeback_slot_ids_cpu
            input_stage_ids_cpu = state.input_stage_ids_cpu
            input_stage_slot_ids_cpu = state.input_stage_slot_ids_cpu
            _used_persistent_input_slots = state.used_persistent_input_slots
        ...
        for name, spec in self.table_specs.items():
            ...
            if name == "lm_head":
                ...
                # (cloud_stage handling removed - always empty)
            ...
            if prefetched_entry is not None:
                prefetched_ids_cpu = prefetched_entry["ids_cpu"]
                if (
                    prefetched_ids_cpu.data_ptr() == stage_ids_for_name.data_ptr()
                    or prefetched_ids_cpu.equal(stage_ids_for_name)
                ):
        ...
        if slot_to_global_cpu is not self.fixed_slot_to_global_cpu:
        ...
        if not _used_persistent_input_slots and not (grad_accum_steps > 1 and not self.disable_fixed_overlap_reuse and preserve_resident_grads):
            self._assert_main_thread_for_slot_mutation("_prepare_fixed_step non-persistent init")
            ...
            if grad_accum_steps > 1:
                if grad_accum_ids_cpu.numel() > 0:
                    ...
                    if not self.disable_fixed_overlap_reuse:
                        ...
                        self._fixed_input_global_to_slot_cpu.fill_(-1)
                        self._fixed_input_global_to_slot_cpu[grad_accum_ids_cpu] = self._grad_accum_slot_positions_view(
                            grad_accum_ids_cpu.numel()
                        )
            else:
        ...
        if active_mask_cpu is not self.fixed_active_mask_cpu:
        ...
        if union_inputs_cpu_local is not None and grad_accum_steps > 1:
            _inputs_local = self._as_cpu_long(union_inputs_cpu_local)
            if self._grad_accum_union_slot_remap_active:
                # Persistent stable slots: remap local-position indices (0..n_local-1 within the
                # current grad-accum union) to the stable GPU slot indices for the window.
                # Eager CPU indexing only — see note at top of file (shapes vary; no compile win).
                _inputs_local = self._grad_accum_wte_local_to_slot_cpu[_inputs_local]
            union_inputs = _inputs_local.to(self.device, non_blocking=self.use_cuda)
        union_targets = None
        if union_targets_cpu_local is not None and grad_accum_steps > 1:
            union_targets = self._as_cpu_long(union_targets_cpu_local).to(self.device, non_blocking=self.use_cuda)
        prep_union_io_h2d_ms = (time.perf_counter() - _t0_union_io) * 1000.0

        _t0_active_vocab = time.perf_counter()
        if use_logit_mask:
            step_active_vocab = self._fixed_active_vocab_with_logit_mask
        else:
            step_active_vocab = self.fixed_active_vocab
        step_optimizer_state = self.fixed_optimizer_state
        # New timer for compile boundary analysis: cost of building the dict passed into the compiled model
        prep_active_vocab_build_ms = (time.perf_counter() - _t0_active_vocab) * 1000.0
        ...
        return DynamicVocabStep(
            ...
            prep_active_vocab_build_ms=prep_active_vocab_build_ms,
            prep_persistent_slot_ms=prep_persistent_slot_ms,
        )
    ...
    def prepare_step(
        ...
        if isinstance(active_ids_cpu, dict):
            ...
            prefetch_hit = 0
            prefetched_persistent_slot_state = None
            prefetched_persistent_slot_wait_ms = 0.0
            prefetched_persistent_slot_hit = 0
            if self.use_cuda and self._pending_stage_prefetch_future is not None and self._pending_stage_prefetch_key == id(active_ids_cpu):
                ...
                self._pending_stage_prefetch_future = None
                self._pending_stage_prefetch_key = None
            if self.use_cuda and self._pending_persistent_slot_future is not None and self._pending_persistent_slot_key == id(active_ids_cpu):
                ps_wait_t0 = time.perf_counter()
                prefetched_persistent_slot_state = self._pending_persistent_slot_future.result()
                prefetched_persistent_slot_wait_ms = (time.perf_counter() - ps_wait_t0) * 1000.0
                prefetched_persistent_slot_hit = 1
                self._pending_persistent_slot_future = None
                self._pending_persistent_slot_key = None
            return self._prepare_fixed_step(
                ...
                prefetched_persistent_slot_state=prefetched_persistent_slot_state,
                prefetched_persistent_slot_wait_ms=prefetched_persistent_slot_wait_ms,
                prefetched_persistent_slot_hit=prefetched_persistent_slot_hit,
            )
    ...
    def apply_accumulated_gradients(self) -> DynamicVocabStep:
        ...
        metrics = DynamicVocabStep(
            ...
            # cold_bias fields removed (dead experimental code)
        )
        self._clear_fixed_grads()
